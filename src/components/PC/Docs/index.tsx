...
const Docs = defineComponent({
  ...
  render() {
    ...
    return (
      <section class="manage-docs-page">
        ...
        <main class="docs-content">
          {['/vant', '/element', '/about'].includes(this.current.path) ? (
            <>{this.current.path === '/about' ? <about-page /> : iframePage(this.current.path)}</>
          ) : (
            component && <component ref={(el: any) => el && (this.mdRef = el)} />
          )}
        </main>
        {Array.isArray(this.current.catalog) && (
          <ul class="docs-catalog">{this.current.catalog}</ul>
        )}
      </section>
    )
  }
