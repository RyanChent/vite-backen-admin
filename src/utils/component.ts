...
export const getSource = (key: string, prop: any, slots: any, emits: any) => {
  ...
  emitsKey.forEach((emitKey) => {
    ...
    source += `\nv-model${newEmitKey === 'modelValue' ? '' : ':' + newEmitKey}=""\n`
  ...
  if (isObject(slots) && Object.keys(slots).length > 0) {
    Object.entries(slots).forEach(([slotKey, value]: any) => {
      if (isFunction(value)) {
        source += `\n<template v-slot:${slotKey}>\n${(value as Function)()}</template>\n`
      } else if (['key', 'prop', 'slots', 'emits'].every((cKey) => value.hasOwnProperty(cKey))) {
        source += `\n<template v-slot:${slotKey}>\n${getSource(
          value.key,
          value.prop,
          value.slots,
...
export const getComponents = (importFile: Array<string>) =>
  importFile
    ...
    .map((str: any) => `${str.split(' ')[1].replaceAll('{', '').replaceAll('}', '')}`)
    .join(`,\n\t  `)
