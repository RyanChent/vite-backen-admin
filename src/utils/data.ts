  ...
  for (const [key, value] of Object.entries(obj)) {
    if (isPrimitiveType(value)) {
      ...
      if (Array.isArray(value)) {
        str += `${"\t".repeat(level)}${key}: ${arrayToString(
          value,
          level + 1
        )},\n`;
        ...
        if (func.includes("function")) {
          ...
          str += `${"\t".repeat(level)}${key}: ${func.replace(
            ` ${funcKey}`,
            ""
          )},\n`;
        } else {
        ...
        str += `${"\t".repeat(level)}${key}: ${objectToString(
          value,
          level + 1
        )},\n`;
      }
...
export const objectToArrayforTree = (
  obj: any,
  prop: any,
  key: any,
  keys: Array<any>
